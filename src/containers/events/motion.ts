...
/**
 * The Meetings & Events hub's scroll choreography, in scroll order — the same
...
export type EventsSectionId = (typeof SCROLL_ORDER)[number];

export const eventsSectionMotion = Object.fromEntries(
  SCROLL_ORDER.map((id, index) => [id, alternatingDirection(index)]),
) as Record<EventsSectionId, RevealDirection>;
