...
const { closing } = legalSections;

export interface LegalClosingSectionProps {
  whatsappHref: string;
  motion?: RevealDirection;
}

/**
 * The last exit, on the dark contrast band: ask on WhatsApp, or call. The
 * band is fixed-dark in both schemes, so the ghost button carries explicit
 * light colours (the same treatment as `FaqClosingSection`).
 */
export function LegalClosingSection({ whatsappHref, motion = "up" }: LegalClosingSectionProps) {
  return (
      ...
      <Reveal direction={motion}>
        <Box sx={{ maxWidth: "60ch", mx: "auto", textAlign: "center" }}>
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 3 }}>
            <ClaimOnWhatsApp href={whatsappHref} label={closing.whatsapp} size="large" />
            <Button
              href={telephoneUrl}
              size="large"
              variant="ghost"
