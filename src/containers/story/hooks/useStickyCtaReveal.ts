...
/**
 * How far the visitor must scroll before the page's sticky "plan a stay" bar
 * appears, in viewport heights. One full screen means the hero — which
 * carries its own primary CTA — is behind them before a second, persistent
 * one slides in, so the two never compete above the fold.
 *
 * A page-local copy of the wellness / events hook of the same name: the
 * threshold reasoning is shared but the constant is a per-funnel decision, so
 * each funnel owns its own rather than importing across containers.
 */
