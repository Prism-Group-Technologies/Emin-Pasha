...
import { useStickyCtaReveal } from "@/containers/story/hooks/useStickyCtaReveal";
import { shadowTokens, zIndexTokens } from "@/theme/tokens";

/**
 * The visible half of the Our Story page's sticky bar. Client because it
 * watches the scroll position (`useStickyCtaReveal`); the WhatsApp href
 * arrives as a plain string from the server wrapper, so the Zod-validated
 * `content` layer never crosses this boundary (DECISIONS.md D25) — the same
 * split the wellness and events sticky bars use.
 *
 ...
 */
export function StoryStickyBar({ whatsappHref }: { whatsappHref: string }) {
  ...
  return (
    ...
    >
      ...
      <Button
        ...
        aria-label={`Book direct on WhatsApp${NEW_TAB_NOTE}`}
      >
        Book on WhatsApp
      </Button>
      <Link href={`#${STAY_ANCHOR_ID}`} variant="body2" underline="hover">
        or send an enquiry
      </Link>
    </Box>
  );
